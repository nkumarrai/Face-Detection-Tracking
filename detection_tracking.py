...
def camshift_tracker(v, file_name):
    ...
    while(1):
        ...
        if frameCounter == 256:
            output.write("%d,%d,%d" % (frameCounter, pt[0], pt[1])) # Write as frame_index,pt_x,pt_y
        else:
...
def particle_tracker(v, file_name):
    ...
    while(1):
        ...
        if frameCounter == 256:
            output.write("%d,%d,%d" % (frameCounter, pos[0], pos[1])) # Write as frame_index,pt_x,pt_y
        else:
...
def kalman_tracker(v, file_name):
    ...
    #print "frame counter", frameCounter, state
...
def of_tracker(v, file_name):
    ...
    while(1):
        ...
        if c == 0 & w == 0 & r == 0 & h == 0:
            ...
            pt_wt_avg = (int(c+w/2), int(r+h/2))
            #cv2.rectangle(frame,(int(pos[0])-w/2, int(pos[1])-h/2),(int(pos[0]+w/2), int(pos[1])+h/2),(0,255,0),3)
            cv2.circle(frame, (int(c+w/2), int(r+h/2)), 5, (0,255,0), -1)
            cv2.imshow('frame', frame)
